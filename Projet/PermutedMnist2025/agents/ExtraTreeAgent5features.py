...
class Agent:
    ...
    def save(self, path: str = "artifacts/ExtraTreeAgent5features"):
        """Sauvegarde le modèle ExtraTrees."""
        try:
            os.makedirs(path, exist_ok=True)
            joblib.dump(self.model, os.path.join(path, "model.pkl"))
            print(f"Agent (ExtraTrees) sauvegardé dans {path}")
        except Exception as e:
            print(f"Erreur lors de la sauvegarde de l'agent : {e}")

    def load(self, path: str = "artifacts/ExtraTreeAgent5features"):
        ...
        try:
            ...
            print(f"Agent (ExtraTrees) chargé depuis {path}")
        except Exception as e:
